  ...
  return (
    <div className="py-4 md:py-6 px-3 md:px-4 max-w-7xl mx-auto animate-[fadeUp_0.4s_ease]">
      <h2 className="font-yatra text-xl md:text-[28px] text-gold-light mb-1 md:mb-1.5 flex items-center gap-2 md:gap-3 bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">
        <Camera className="text-gold-light w-6 h-6 md:w-7 md:h-7" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 md:gap-5">
        {SPOTS.map((spot, index) => (
          <div 
            ...
            className="bg-white/5 backdrop-blur-md rounded-xl md:rounded-2xl overflow-hidden border border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.2)] transition-all duration-300 cursor-pointer hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(201,148,26,0.15)] group flex flex-col"
          >
            <div className="h-[100px] md:h-[180px] relative overflow-hidden shrink-0 border-b border-white/10">
              <img 
                src={spot.image} 
                alt={spot.name}
            ...
            <div className="p-2.5 md:p-4 flex-1 flex flex-col bg-gradient-to-b from-white/5 to-transparent">
              <h3 className="font-yatra text-[13px] md:text-[17px] text-slate-200 mb-0.5 md:mb-1 leading-tight group-hover:text-gold-light transition-colors">{spot.name}</h3>
              <div className="font-malayalam text-[10px] md:text-[13px] text-gold/80 mb-1 md:mb-2">{spot.mal}</div>
              <p className="text-[10px] md:text-[13px] text-slate-400 leading-[1.5] line-clamp-3 md:line-clamp-none">{spot.desc}</p>
            </div>
          </div>
        ))}
      </div>
