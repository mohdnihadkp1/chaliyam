  ...
  return (
    <div className="py-10 px-6 max-w-7xl mx-auto animate-[fadeUp_0.4s_ease]">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          <h2 className="font-yatra text-[28px] text-gold-light mb-1.5 bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 px-4 py-2 rounded-xl font-medium transition-colors border border-red-500/30 text-sm backdrop-blur-md shadow-inner"
        >
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-72 shrink-0 bg-white/5 rounded-2xl p-5 border border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.2)] h-fit backdrop-blur-md">
          <div className="flex items-center gap-2 text-gold-light font-semibold text-lg mb-4 pb-3 border-b border-white/10">
            <Layers size={20} />
            Map Layers
          </div>
          ...
          <div className="flex flex-col gap-3">
            {/* Map Type Toggle */}
            <div className="flex items-center gap-2 bg-slate-900/50 p-1.5 rounded-xl mb-2 border border-white/5 shadow-inner">
              <button
                onClick={() => setMapType('standard')}
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-colors ${mapType === 'standard' ? 'bg-white/10 text-gold-light shadow-sm border border-white/10' : 'text-slate-400 hover:text-slate-200'}`}
              >
                <MapIcon size={16} />
                Standard
              </button>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-colors ${mapType === 'satellite' ? 'bg-white/10 text-gold-light shadow-sm border border-white/10' : 'text-slate-400 hover:text-slate-200'}`}
              >
                <Globe size={16} />
                Satellite
              </button>
            ...
            <div 
              ...
              className={`flex items-center justify-between cursor-pointer p-3 rounded-xl border transition-all duration-200 ${showSpots ? 'bg-gold/10 border-gold/30 shadow-[0_0_15px_rgba(201,148,26,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${showSpots ? 'bg-gold text-slate-900' : 'bg-slate-800 text-slate-400 border border-white/5'}`}>
                  <MapPin size={showSpots ? 20 : 18} className={showSpots ? 'stroke-[2.5px]' : ''} />
                </div>
                <div className="flex flex-col">
                  <span className={`text-sm font-semibold ${showSpots ? 'text-gold-light' : 'text-slate-300'}`}>Popular Spots</span>
                  <div className="group relative inline-block">
                    <span className="text-[11px] text-slate-500 flex items-center gap-1 hover:text-gold-light transition-colors">
                      <Info size={12} /> What's this?
                    </span>
                    <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-slate-900 border border-white/10 text-slate-200 text-xs rounded shadow-xl z-50 backdrop-blur-md">
                      Displays key locations, beaches, and landmarks across the village.
                    </div>
                  </div>
                </div>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${showSpots ? 'border-gold bg-gold' : 'border-slate-600'}`}>
                {showSpots && <div className="w-2 h-2 bg-slate-900 rounded-full" />}
              </div>
            </div>
            ...
            <div 
              ...
              className={`flex items-center justify-between cursor-pointer p-3 rounded-xl border transition-all duration-200 ${showEmergency ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${showEmergency ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-400 border border-white/5'}`}>
                  <ShieldAlert size={showEmergency ? 20 : 18} className={showEmergency ? 'stroke-[2.5px]' : ''} />
                </div>
                <div className="flex flex-col">
                  <span className={`text-sm font-semibold ${showEmergency ? 'text-red-400' : 'text-slate-300'}`}>Emergency</span>
                  <div className="group relative inline-block">
                    <span className="text-[11px] text-slate-500 flex items-center gap-1 hover:text-red-400 transition-colors">
                      <Info size={12} /> What's this?
                    </span>
                    <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-slate-900 border border-white/10 text-slate-200 text-xs rounded shadow-xl z-50 backdrop-blur-md">
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${showEmergency ? 'border-red-500 bg-red-500' : 'border-slate-600'}`}>
            ...
            <div 
              ...
              className={`flex items-center justify-between cursor-pointer p-3 rounded-xl border transition-all duration-200 ${showTransport ? 'bg-blue-500/10 border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${showTransport ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400 border border-white/5'}`}>
                  <Bus size={showTransport ? 20 : 18} className={showTransport ? 'stroke-[2.5px]' : ''} />
                </div>
                <div className="flex flex-col">
                  <span className={`text-sm font-semibold ${showTransport ? 'text-blue-400' : 'text-slate-300'}`}>Transport Hubs</span>
                  <div className="group relative inline-block">
                    <span className="text-[11px] text-slate-500 flex items-center gap-1 hover:text-blue-400 transition-colors">
                      <Info size={12} /> What's this?
                    </span>
                    <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-slate-900 border border-white/10 text-slate-200 text-xs rounded shadow-xl z-50 backdrop-blur-md">
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${showTransport ? 'border-blue-500 bg-blue-500' : 'border-slate-600'}`}>
          ...
          <div className="mt-6 pt-4 border-t border-white/10">
            <button 
              ...
              className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl transition-colors font-medium text-sm border mb-3 ${isOfflineReady ? 'bg-green-500/20 hover:bg-green-500/30 text-green-400 border-green-500/40' : 'bg-white/5 hover:bg-white/10 text-slate-300 border-white/10'}`}
            >
        ...
        {/* Map Container */}
        <div className="flex-1 bg-white/5 rounded-2xl p-2 border border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.2)] backdrop-blur-md">
          <div className="relative w-full h-[500px] md:h-[600px] rounded-xl overflow-hidden z-0 border border-white/5">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl border border-white/10 animate-[fadeUp_0.3s_ease]">
            <div className="flex justify-between items-center p-4 border-b border-white/10 bg-slate-800/50">
              ...
              <button onClick={() => setIsReportModalOpen(false)} className="text-slate-400 hover:text-red-400 transition-colors bg-white/5 p-1.5 rounded-full border border-white/10">
                <X size={18} />
              </button>
            </div>
            <form onSubmit={handleReportSubmit} className="p-5">
              <div className="flex gap-2 mb-4">
                <button 
                  ...
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${reportType === 'issue' ? 'bg-red-500/20 text-red-400 border border-red-500/30 shadow-inner' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'}`}
                >
                  Report Issue
                </button>
                <button 
                  ...
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${reportType === 'landmark' ? 'bg-green-500/20 text-green-400 border border-green-500/30 shadow-inner' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'}`}
                >
                  Suggest Landmark
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1">Location Details</label>
                  <input name="location" required type="text" placeholder="e.g., Near Chaliyam Beach" className="w-full px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-slate-200 outline-none focus:border-gold/50 focus:bg-white/10 placeholder:text-slate-500 shadow-inner" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1">Description</label>
                  <textarea name="desc" required rows={4} placeholder={reportType === 'issue' ? "Describe the map issue..." : "Describe the landmark..."} className="w-full px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-slate-200 outline-none focus:border-gold/50 focus:bg-white/10 resize-none placeholder:text-slate-500 shadow-inner"></textarea>
                </div>
                <button 
                  type="submit"
                  className="w-full bg-[#25D366]/90 hover:bg-[#25D366] border border-[#25D366]/50 text-white py-2.5 rounded-lg font-medium transition-colors shadow-[0_0_15px_rgba(37,211,102,0.2)]"
                >
                  Submit via WhatsApp
                </button>
              </div>
      ...
      {showOfflineModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl border border-white/10 animate-[fadeUp_0.3s_ease]">
            <div className="flex justify-between items-center p-4 border-b border-white/10 bg-slate-800/50">
              <h3 className="font-semibold text-lg text-slate-200 flex items-center gap-2">
                <Download size={20} className="text-gold-light" />
                Offline Maps
              </h3>
              <button onClick={() => setShowOfflineModal(false)} className="text-slate-400 hover:text-red-400 transition-colors bg-white/5 p-1.5 rounded-full border border-white/10">
                <X size={18} />
              </button>
            </div>
            <div className="p-5">
              {isOfflineReady && !isDownloading ? (
                <div className="bg-green-500/10 border border-green-500/20 text-green-400 p-4 rounded-xl mb-4 text-sm leading-relaxed flex items-start gap-3 shadow-inner">
                  ...
                  <div>
                    <p className="font-semibold mb-1 text-green-300">Map is available offline!</p>
                ...
                <>
                  <div className="bg-blue-500/10 border border-blue-500/20 text-blue-300 p-4 rounded-xl mb-4 text-sm leading-relaxed shadow-inner">
                    <p className="mb-2 text-blue-200"><strong>Download Map for Offline Use</strong></p>
                  ...
                  {isDownloading ? (
                    <div className="mb-4">
                      ...
                      <div className="w-full bg-slate-800 rounded-full h-2.5 border border-white/5 shadow-inner">
                        <div className="bg-gold h-2.5 rounded-full transition-all duration-300" style={{ width: `${downloadProgress}%` }}></div>
                      </div>
                    </div>
                  ) : (
                    <button 
                      onClick={downloadOfflineMap}
                      className="w-full bg-gold/90 hover:bg-gold text-slate-900 border border-gold/50 py-2.5 rounded-lg font-medium transition-colors mb-3 flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(201,148,26,0.2)]"
                    >
                      <Download size={18} />
                      Start Download
                    </button>
              ...
              <button 
                ...
                className="w-full bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 py-2.5 rounded-lg font-medium transition-colors disabled:opacity-50"
              >
                Close
              </button>
            </div>
