...
import { Layers, AlertTriangle, X, Home, Palmtree, Anchor, Waves, ShieldAlert, Ship, Bus, Landmark, Trees, MapPin, Info, Map as MapIcon, Download, Building, Globe, CheckCircle2, Loader2, Navigation } from 'lucide-react';
  ...
  return (
    <div className="py-4 md:py-6 px-3 md:px-4 max-w-7xl mx-auto animate-[fadeUp_0.4s_ease]">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 md:gap-4 mb-4 md:mb-6">
        <div>
          <h2 className="font-yatra text-xl md:text-[28px] text-gold-light mb-1 md:mb-1.5 bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-1.5 md:gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 px-3 py-2 md:px-4 md:py-2 rounded-lg md:rounded-xl font-medium transition-colors border border-red-500/30 text-xs md:text-sm backdrop-blur-md shadow-inner"
        >
      ...
      <div className="flex flex-col-reverse lg:flex-row gap-4 md:gap-6">
        ...
        <div className="w-full lg:w-72 shrink-0 bg-white/5 rounded-xl md:rounded-2xl p-3 md:p-5 border border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.2)] h-fit backdrop-blur-md">
          <div className="flex items-center justify-between text-gold-light font-semibold text-sm md:text-lg mb-2 md:mb-4 pb-2 md:pb-3 border-b border-white/10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-1 gap-2 md:gap-3">
            {/* Map Type Toggle */}
            <div className="col-span-2 lg:col-span-1 flex items-center gap-1.5 md:gap-2 bg-slate-900/50 p-1 md:p-1.5 rounded-lg md:rounded-xl mb-1 border border-white/5 shadow-inner">
              <button
                onClick={() => setMapType('standard')}
                className={`flex-1 flex items-center justify-center gap-1.5 md:gap-2 py-1.5 md:py-2 rounded md:rounded-lg text-xs md:text-sm font-medium transition-colors ${mapType === 'standard' ? 'bg-white/10 text-gold-light shadow-sm border border-white/10' : 'text-slate-400 hover:text-slate-200'}`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 md:gap-2 py-1.5 md:py-2 rounded md:rounded-lg text-xs md:text-sm font-medium transition-colors ${mapType === 'satellite' ? 'bg-white/10 text-gold-light shadow-sm border border-white/10' : 'text-slate-400 hover:text-slate-200'}`}
              >
            ...
            <div 
              ...
              className={`flex items-center justify-between cursor-pointer p-2 md:p-3 rounded-lg md:rounded-xl border transition-all duration-200 ${showSpots ? 'bg-gold/10 border-gold/30 shadow-[0_0_15px_rgba(201,148,26,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
            >
              <div className="flex items-center gap-2 md:gap-3">
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors ${showSpots ? 'bg-gold text-slate-900' : 'bg-slate-800 text-slate-400 border border-white/5'}`}>
                  <MapPin size={showSpots ? 16 : 14} className={showSpots ? 'stroke-[2.5px] md:w-5 md:h-5' : 'md:w-[18px] md:h-[18px]'} />
                </div>
                <div className="flex flex-col">
                  <span className={`text-xs md:text-sm font-semibold ${showSpots ? 'text-gold-light' : 'text-slate-300'}`}>Spots</span>
                </div>
              </div>
            </div>
            ...
            <div 
              ...
              className={`flex items-center justify-between cursor-pointer p-2 md:p-3 rounded-lg md:rounded-xl border transition-all duration-200 ${showEmergency ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
            >
              <div className="flex items-center gap-2 md:gap-3">
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors ${showEmergency ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-400 border border-white/5'}`}>
                  <ShieldAlert size={showEmergency ? 16 : 14} className={showEmergency ? 'stroke-[2.5px] md:w-5 md:h-5' : 'md:w-[18px] md:h-[18px]'} />
                </div>
                <div className="flex flex-col">
                  <span className={`text-xs md:text-sm font-semibold ${showEmergency ? 'text-red-400' : 'text-slate-300'}`}>Emergency</span>
                </div>
              </div>
            </div>
            ...
            <div 
              ...
              className={`flex items-center justify-between cursor-pointer p-2 md:p-3 rounded-lg md:rounded-xl border transition-all duration-200 col-span-2 lg:col-span-1 ${showTransport ? 'bg-blue-500/10 border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
            >
              <div className="flex items-center gap-2 md:gap-3">
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors ${showTransport ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400 border border-white/5'}`}>
                  <Bus size={showTransport ? 16 : 14} className={showTransport ? 'stroke-[2.5px] md:w-5 md:h-5' : 'md:w-[18px] md:h-[18px]'} />
                </div>
                <div className="flex flex-col">
                  <span className={`text-xs md:text-sm font-semibold ${showTransport ? 'text-blue-400' : 'text-slate-300'}`}>Transport Hubs</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Map Container */}
        <div className="flex-1 bg-white/5 rounded-xl md:rounded-2xl p-1 md:p-2 border border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.2)] backdrop-blur-md relative">
          ...
          <button
            ...
            className="absolute bottom-6 right-6 z-10 bg-white text-slate-900 p-3 rounded-full shadow-lg hover:bg-slate-100 transition-colors flex items-center justify-center"
            title="My Location"
          >
