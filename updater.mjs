...
function updateSection(file, listName, type, jsonLdFunc, idField = "id") {
  ...
  const stateRegex = /const\s+\[(selectedItem|selectedSpot|shareModal),\s*(setSelectedItem|setSelectedSpot|setShareModal)\]\s*=\s*useState<.*?>\(null\);/;
  ...
  if (!code.includes('<Helmet>') && returnRegex.test(code)) {
    ...
    // Check if the file is NewsSection, the listName might be NEWS_UPDATES depending on the context
...
updateSection('src/components/NewsSection.tsx', 'NEWS_UPDATES', 'Event', `{ "@context": "https://schema.org", "@type": "Event", name: shareModal.title, description: shareModal.desc, startDate: shareModal.date, location: { "@type": "Place", name: "Chaliyam" } }`);
